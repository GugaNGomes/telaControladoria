...
const exportarParaExcel = (dados, nomeArquivo = "relatorio") => {
  try {
    // Remove as colunas que não devem ser exportadas e processa os valores
    const dadosExport = dados.map((row) => {
      const copia = { ...row };
      delete copia.valorDocumentoNumero;
      delete copia.valorDescontoAcrescimoNumero;
      
      // Processa cada campo para remover formatação desnecessária
      for (const key in copia) {
        if (typeof copia[key] === "string") {
          let valor = copia[key];
          
          // Remove 'R$' dos valores monetários e converte para número
          if (valor.includes("R$")) {
            // Remove R$, espaços, pontos (milhares) e substitui vírgula por ponto (decimal)
            ...
            // Se conseguiu converter para número, usa o valor numérico
            const num = parseFloat(valorNumerico);
            if (!isNaN(num)) {
              copia[key] = num;
            } else {
              copia[key] = valor.replace(/R\$\s?/g, "").replace(/\u00A0/g, "").trim();
            }
          } else {
            copia[key] = valor.trim();
          }
        }
      }
      return copia;
    ...
    // Cria um mapeamento de nomes de colunas mais legíveis
    const colunasMapeadas = dadosExport.map(row => ({
      "Número do Documento": row.numeroDocumento,
      "Sigla": row.tipoAbertoPorLinha,
      "Data de Emissão": row.dataEmissaoDocumento,
      "Competência do Serviço": row.competenciaComercial,
      "Cliente": row.cliente,
      "Galpão": row.galpao,
      "Solicitante": row.solicitante,
      "Fatura": row.fatura,
      "Nota Fiscal": row.notaFiscal,
      "Valor do Documento": row.valorDocumento,
      "Tipo de Faturamento": row.tipoFaturamento
    }));

    // Usa a biblioteca XLSX para gerar arquivo Excel nativo
    const ws = XLSX.utils.json_to_sheet(colunasMapeadas);
    
    // Ajusta a largura das colunas automaticamente
    const wscols = Object.keys(colunasMapeadas[0] || {}).map(() => ({ wch: 25 }));
    ws['!cols'] = wscols;
    
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Relatório Controladoria");
    
    // Gera arquivo Excel (.xlsx) sem aspas desnecessárias
...
export default function Tabela({ dados = [], evolucaoAnual = [] }) {
    ...
    return dados.filter((item) => {
      for (const coluna in filtrosTabela) {
        if (coluna === "dataEmissaoDocumento" && filtrosTabela[coluna]) {
          ...
          const intervaloMatch = valor.match(
          ...
          if (intervaloMatch) {
            ...
            if (
              item.dataEmissaoDocumento < inicio ||
              item.dataEmissaoDocumento > fim
            ) {
              return false;
            }
          } else {
            ...
            if (
              !String(item.dataEmissaoDocumento || "")
                .toLowerCase()
          ...
          if (
            String(item[coluna] || "")
              .toLowerCase()
              .indexOf(filtrosTabela[coluna].toLowerCase()) === -1
          ) {
            return false;
          }
        }
  ...
  const calcularDadosRelatorio = () => {
    ...
    const itensFaturados = dados.filter(
      (item) => item.fatura && item.fatura !== "-"
    );
    const itensNaoFaturados = dados.filter(
      (item) => !item.fatura || item.fatura === "-"
    );
    ...
    const percentualFaturado =
      totalDocumentos > 0
        ? ((totalFaturados / totalDocumentos) * 100).toFixed(1)
        : "0.0";
    const percentualNaoFaturado =
      totalDocumentos > 0
        ? ((totalNaoFaturados / totalDocumentos) * 100).toFixed(1)
        : "0.0";
    ...
    <div>
      ...
      <div className="botoes-navegacao">
        <button
          className={`btn-navegacao ${
            visualizacaoAtiva === "grafico" ? "ativo" : ""
          }`}
        ...
        <button
          className={`btn-navegacao ${
            visualizacaoAtiva === "tabela" ? "ativo" : ""
          }`}
            ...
            <table className="tabela-principal">
                  ...
                  <th>
                    ...
                    <div>
                      <FiltrosInput
                        ...
                        onChange={(e) =>
                          handleFiltroChange("numeroDocumento", e.target.value)
                        }
                      />
                    </div>
                  </th>
                  ...
                  <th>
                    ...
                    <div>
                      <FiltrosInput
                        ...
                        onChange={(e) =>
                          handleFiltroChange("galpao", e.target.value)
                        }
                      />
                    </div>
                  </th>
                  ...
                  <th>
                    ...
                    <div>
                      <FiltrosInput
                        ...
                        onChange={(e) =>
                          handleFiltroChange("fatura", e.target.value)
                        }
                      />
                    </div>
                  </th>
                  <th>
                    ...
                    <div>
                      <FiltrosInput
                        ...
                        onChange={(e) =>
                          handleFiltroChange("notaFiscal", e.target.value)
                        }
                      />
                    </div>
                  </th>
                  ...
                  <th>
                    ...
                    <div>
                      <FiltrosInput
                        ...
                        onChange={(e) =>
                          handleFiltroChange("tipoFaturamento", e.target.value)
                        }
                      />
                    </div>
                  </th>
              ...
              <tbody>
                {dadosPaginaAtual.map((item, index) => (
                  <tr key={inicio + index}>
                    <td style={{ color: "#477ABE", fontWeight: "bold" }}>
                      {item.numeroDocumento}
                    </td>
                    <td>{item.tipoAbertoPorLinha}</td>
                    <td>{formatarData(item.dataEmissaoDocumento)}</td>
                    <td>
                      {formatarCompetenciaComercial(item.competenciaComercial)}
                    </td>
                    <td>{item.cliente}</td>
                    <td>{item.galpao}</td>
              ...
              <div className="paginacao-esquerda">
                <div className="seletor-linhas">
                  <div className="paginacao-info">
                    ...
                    {Math.min(fim, dadosFiltrados.length)} de{" "}
                    {dadosFiltrados.length} Itens
                  </div>
                  <select
                    id="itensPorPagina"
      ...
      {visualizacaoAtiva === "grafico" && (
        <div className="grafico-container">
          <RelatorioControladoria
            dados={calcularDadosRelatorio()}
